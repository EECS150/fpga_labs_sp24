...
- [wire vs reg](https://www.eecs151.org/files/verilog/wire_vs_reg.pdf) - the differences between `wire` and `reg` nets and when to use each one
- [always @ blocks](https://www.eecs151.org/files/verilog/always_at_blocks.pdf) - understanding the differences between the two types of `always @` blocks and what they synthesize to 
  - *Note: This does not adhere to the no flip-flop inference policy enforced in this class, i.e. you should not use `always @(posedge clk)` blocks, although they are important to learn about!*

<details open>

<summary> Table of Contents (click to close) </summary>

- [FPGA Lab 3: More Sequential Circuits, Audio "DAC"](#fpga-lab-3-more-sequential-circuits-audio-dac)
  - [Overview](#overview)
  - [Before You Begin](#before-you-begin)
    - [Lab Video Walkthrough](#lab-video-walkthrough)
    - [Verilog Reading](#verilog-reading)
  - [Additional Testbench Constructs](#additional-testbench-constructs)
  - [Input Conditioning Circuits](#input-conditioning-circuits)
    - [Synchronizer](#synchronizer)
    - [Edge Detector](#edge-detector)
    - [Debouncer](#debouncer)
      - [Debouncer Implementation Notes](#debouncer-implementation-notes)
      - [Debouncer Testbench Notes](#debouncer-testbench-notes)
    - [Button Parser on the FPGA](#button-parser-on-the-fpga)
      - [Counter with Button Parser](#counter-with-button-parser)
      - [Counter Testbench](#counter-testbench)
      - [Play and Pause your Counter](#play-and-pause-your-counter)
  - [PWM](#pwm)
    - [Audio Out](#audio-out)
    - [PWM Generator as DAC](#pwm-generator-as-dac)
    - [DAC Testbench](#dac-testbench)
    - [Square Wave Generator](#square-wave-generator)
    - [Square Wave Generator Testbench](#square-wave-generator-testbench)
...
At a high level, the debouncer ensures that you are only outputting a 1 if the glitchy signal has been held high for a given amount of samples (`PULSE_CNT_MAX`) where each sample occurs every `SAMPLE_CNT_MAX` cycles.

Start by thinking about how to implement the debouncer for `WIDTH=1` and then try to extend the logic to a parametrizable input width.
...
If you are seeing issues where certain registers are red lines (X's), make sure you give them an initial state and that they aren't being driven by different sources (multi-driven net).
...
**For Windows Users**: You may need to change `rtoi` to `floor` instead in `src/z1top.v`
...
We'd like suggest that you **think about the structure of your design ahead of coding**. The debouncer circuit figure above is a great example. Remember that Verilog coding is all about describing your hardware circuit (very unlike previous imperative programming languages you may be used to)! Sketch a block diagram of your mode counter circuit in terms of the `button_parser` block and some register blocks from `EECS151.v` as you see fit. Label the relevant input and output signals. Feel free to use any logic gates, MUXes, or adders. Don't worry about listing all the details.

## PWM 
Staff has put together a conceptual walkthrough for the DAC, and square wave generator: https://youtu.be/1FiaAPFCMFU  (This is the same as the beginning of the lab, so you can skip if you have already watched it)
...
To checkoff for this lab, have these things ready to show the TA:
  ...
  - Be ready to explain what order the button parser connects the input conditioning circuits
